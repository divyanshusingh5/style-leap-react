    ...
    <div className="space-y-6">
      ...
      <Card className="border-warning/20">
        <CardHeader>
          ...
          <CardDescription>Evaluate whether severity and causation factor weights need recalibration</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Causation Factors */}
          <div>
            <h4 className="font-semibold mb-3 text-lg">Causation Factors Performance</h4>
            <div className="space-y-4">
              {(() => {
                const causationFactors = [
                  { name: 'Probability Incident Caused Injury', field: 'causation_probability' as const, weights: [0.3257, 0.2212, 0.4478, 0.0000] },
                  { name: 'Unreasonable Initial Tx Delay', field: 'causation_tx_delay' as const, weights: [0.1226, 0.0000] },
                  { name: 'Unreasonable Tx Gaps', field: 'causation_tx_gaps' as const, weights: [0.1313, 0.0000] },
                  { name: 'Compliance', field: 'causation_compliance' as const, weights: [0.1474, 0.0864] }
                ];

                return causationFactors.map(factor => {
                  // Group by weight value and calculate avg variance
                  const weightGroups = new Map<number, number[]>();
                  data.forEach(claim => {
                    const weight = claim[factor.field];
                    if (!weightGroups.has(weight)) weightGroups.set(weight, []);
                    weightGroups.get(weight)!.push(Math.abs(claim.variance_pct));
                  });

                  const weightAnalysis = Array.from(weightGroups.entries()).map(([weight, variances]) => ({
                    weight,
                    avgVariance: variances.reduce((sum, v) => sum + v, 0) / variances.length,
                    count: variances.length,
                    maxWeight: Math.max(...factor.weights),
                    isHighWeight: weight === Math.max(...factor.weights)
                  })).sort((a, b) => b.avgVariance - a.avgVariance);

                  const highestVarianceWeight = weightAnalysis[0];
                  const needsRecalibration = highestVarianceWeight.avgVariance > 25 || 
                    (highestVarianceWeight.isHighWeight && highestVarianceWeight.avgVariance > 20);

                  return (
                    <div key={factor.field} className={`border rounded-lg p-4 ${needsRecalibration ? 'border-warning bg-warning/5' : 'border-border'}`}>
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <div className="font-medium">{factor.name}</div>
                          <div className="text-xs text-muted-foreground mt-1">
                            Weight range: {Math.min(...factor.weights).toFixed(4)} - {Math.max(...factor.weights).toFixed(4)}
                          </div>
                        </div>
                        {needsRecalibration && <Badge variant="destructive">Needs Review</Badge>}
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
                        {weightAnalysis.map(wa => (
                          <div key={wa.weight} className={`bg-muted/30 rounded p-2 ${wa === highestVarianceWeight && needsRecalibration ? 'ring-2 ring-warning' : ''}`}>
                            <div className="text-xs text-muted-foreground">Weight: {wa.weight.toFixed(4)}</div>
                            <div className="font-semibold text-sm">{wa.avgVariance.toFixed(1)}% avg var</div>
                            <div className="text-xs">{wa.count} claims</div>
                          </div>
                        ))}
                      </div>
                      {needsRecalibration && (
                        <div className="bg-warning/10 rounded p-2 text-sm">
                          <span className="font-medium">⚠️ Recommendation: </span>
                          Weight {highestVarianceWeight.weight.toFixed(4)} shows {highestVarianceWeight.avgVariance.toFixed(1)}% average variance across {highestVarianceWeight.count} claims. 
                          Consider adjusting this weight or investigating claims in this category for common patterns.
                        </div>
                      )}
                    </div>
                  );
          ...
          {/* Severity Factors */}
          <div>
            <h4 className="font-semibold mb-3 text-lg">Severity Factors Performance</h4>
            <div className="space-y-4">
              {(() => {
                const severityFactors = [
                  { name: 'Allowed Tx Period', field: 'severity_allowed_tx_period' as const, weights: [1.4488, 0.0000, 2.3490, 3.1606, 3.8533, 4.3507] },
                  { name: 'Initial Tx', field: 'severity_initial_tx' as const, weights: [1.5445, 1.2406, 0.6738, 0.0000] },
                  { name: 'Injections', field: 'severity_injections' as const, weights: [0.0000, 1.9855, 3.0855, 3.4370, 5.1228] },
                  { name: 'Objective Findings', field: 'severity_objective_findings' as const, weights: [2.7611, 0.0000] },
                  { name: 'Pain Management', field: 'severity_pain_mgmt' as const, weights: [0.0000, 0.6396, 1.1258] },
                  { name: 'Type of Treatment', field: 'severity_type_tx' as const, weights: [2.0592, 3.2501] },
                  { name: 'Injury Site', field: 'severity_injury_site' as const, weights: [1.8450, 1.1767, 0.9862, 0.4866, 0.0000] },
                  { name: 'Severity Code', field: 'severity_code' as const, weights: [0.4864, 0.3803, 0.8746, 0.0000] }
                ];
                    ...
                    <div key={factor.field} className={`border rounded-lg p-4 ${needsRecalibration ? 'border-warning bg-warning/5' : 'border-border'}`}>
                      ...
                      {needsRecalibration && (
                        <div className="bg-warning/10 rounded p-2 text-sm">
                          <span className="font-medium">⚠️ Recommendation: </span>
                          Weight {highestVarianceWeight.weight.toFixed(4)} shows {highestVarianceWeight.avgVariance.toFixed(1)}% average variance across {highestVarianceWeight.count} claims. 
                          {highestVarianceWeight.isHighWeight ? 
                            ' This is the highest weight in this factor, suggesting the model may be over-emphasizing this category.' :
                            ' Consider adjusting this weight or investigating claims in this category for common patterns.'}
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-info/10 border border-info/20 rounded-lg p-4">
            <h4 className="font-semibold mb-2 flex items-center gap-2">
              ...
              Factor Weight Recalibration Summary
            </h4>
            <p className="text-sm">
              The analysis above shows variance patterns for each factor weight. Factors marked "Needs Review" show concerning variance levels 
              (either &gt;25% overall or &gt;20% for highest weights). These weights should be prioritized for recalibration. Focus on:
            </p>
            <ul className="text-sm mt-2 space-y-1 list-disc list-inside">
              <li>Weights with highest variance-to-weight ratios</li>
              <li>High-weight categories that still show elevated variance</li>
              <li>Weights where claims are concentrated but performance is poor</li>
            </ul>
          </div>
        </CardContent>
      </Card>
